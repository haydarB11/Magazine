...
class MagazineController {
    ...
    findMagazinesForOnePublisher = async (req, res, next) => {
    ...
    findMagazinesForOneCollection = async (req, res, next) => {
        const { collection_id } = req.params;
        try {
            const magazines = await MagazineService.findMagazinesForOneCollection(collection_id);
            return JsonResponse.success(res, magazines);
        } catch (error) {
            next(error)
        }
    ...
    deleteManyMagazines = async (req, res, next) => {
        ...
        try {
            ...
            return JsonResponse.success(res, magazines, i18next.t("delete_message", { field: "Magazine" }), HttpConstant.OK);
        } catch (error) {
            next(error)
        }
    };
